class game:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.passed == False:
                    ...
                    if ((self.board.boardState[self.simpleMousePos[1]][self.simpleMousePos[0]][1] != None) and
                        (self.board.boardState[self.simpleMousePos[1]][self.simpleMousePos[0]][0] == self.playerColor) and
    ...
    def checkSomeoneWon(self): 
        for column in range(8):
            for row in range(8):
                if self.AiTurn == False and self.board.boardStatee[row][column][0] == self.playerColour:
                    if self.board.availableMoves((column, row), self.passed) != []:
                        return False
                if self.Ai == True and self.AiTurn == True:
                    if self.board.boardStatee[row][column][0] == self.aiColour: #changed from red to self.aiColour
                        if self.board.availableMoves((column, row), self.passed) != []:
                            return False
    ...
    def whoWon(self, player):
        if player == "Ai":
            for column in range(8):
                for row in range(8):
                    if self.board.boardStatee[row][column][0] == self.playerColour: #changed from black to self.playerColour
                        if self.board.availableMoves((column, row), self.passed) != []:
                            return False
            return True
    ...
    def evaluate(self, Colour):
        ...
        for column in range(8):
            for row in range(8):
                ...
                if self.board.boardStatee[row][column][0] == self.aiColour: #changed from red to self.aiColour
                    if self.board.boardStatee[row][column][1].stat == "king":
                        redKingsNum += 1
                    else:
                        redNum += 1
                if self.board.boardStatee[row][column][0] == self.playerColour: #changed from black to self.playerColour
        ...
        if player == "Ai":
            for piece in updateAiPieces:
                ...
                for move in updateAiPieces[piece]:
                    originalPos = piece
                    originalStat = [self.board.boardStatee[piece[1]][piece[0]][0],self.board.boardStatee[piece[1]][piece[0]][1]]
